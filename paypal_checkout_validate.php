...
if(!empty($_POST['paypal_order_check']) && !empty($_POST['order_id'])){ 
    ...
    if(!empty($order)){ 
        ...
        if(!empty($order['purchase_units'][0])){ 
            ...
            $item_number = empty($purchase_unit['custom_id'])?'123':$purchase_unit['custom_id'];
            
    
            $item_name = empty($purchase_unit['description'])?'digital product':$purchase_unit['description']; 
        ...
        if(!empty($order['payer'])){ 
            ...
            $payer_country_code = !empty($payer_address['country_code'])?$payer_address['country_code']:''; 
             ...
             // Check if the phone number is available
                $payer_phone = !empty($payer['phone']) ? $payer['phone'] : null;
                if ($payer_phone) {
                    $payer_phone_number = !empty($payer_phone['phone_number']) ? $payer_phone['phone_number'] : null;
                    if ($payer_phone_number) {
                        $payer_phone_number_national = !empty($payer_phone_number['national_number']) ? $payer_phone_number['national_number'] : '';
        ...
        if(!empty($order_id) && $order_status == 'COMPLETED'){ 
            ...
            if(!empty($row_id)){ 
                ...
                $sqlQ = "INSERT INTO transactions (item_number,item_name,item_price,item_price_currency,payer_id,payer_name,payer_email,payer_country,merchant_id,merchant_email,order_id,transaction_id,paid_amount,paid_amount_currency,payment_source,payment_status,created,modified,phone) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW(),?)"; 
                $stmt = $db->prepare($sqlQ); 
                $stmt->bind_param("ssdsssssssssdsssss", $item_number, $item_name, $itemPrice, $currency, $payer_id, $payer_full_name, $payer_email_address, $payer_country_code, $merchant_id, $payee_email_address, $order_id, $transaction_id, $amount_value, $currency_code, $payment_source, $payment_status, $order_time,$payer_phone_number_national); 
                $insert = $stmt->execute(); 
